...
class LocationRequirement(BaseModel):
    ...
    state: Optional[str] = Field(
        None, description="State/province where the job is located"
    )

    country: str = Field(
        "United States", description="Country where the job is located"
    )

    timezone_requirements: Optional[str] = Field(
        None, description="Required timezone overlap for remote work"
    )

    travel_required: bool = Field(
        False, description="Whether travel is required for this position"
    )

    travel_percentage: Optional[int] = Field(
        None, ge=0, le=100, description="Percentage of time spent traveling"
    )
...
class ExperienceRequirement(BaseModel):
    ...
    level: ExperienceLevel = Field(..., description="Required experience level")

    years_min: int = Field(
        0, ge=0, le=50, description="Minimum years of experience required"
    )

    years_max: Optional[int] = Field(
        None, ge=0, le=50, description="Maximum years of experience (for targeting)"
    )

    industry_experience: Optional[str] = Field(
        None, description="Specific industry experience requirements"
    )

    leadership_required: bool = Field(
        False, description="Whether leadership experience is required"
    )

    management_experience: bool = Field(
        False, description="Whether people management experience is required"
    )
    ...
    def validate_experience_consistency(self) -> "ExperienceRequirement":
        ...
        if self.years_min > 0 and self.years_min > max_expected:
            raise ValueError(
                f"{self.level.value} level typically requires {min_expected}-{max_expected} years"
            )
...
class Skill(BaseModel):
    ...
    skill_type: SkillType = Field(..., description="Type/category of skill")

    is_required: bool = Field(
        True, description="Whether this skill is required or preferred"
    )
...
class SkillsRequirement(BaseModel):
    ...
    technical_skills: List[Skill] = Field(
        default_factory=list, description="Technical skills required for the position"
    )

    soft_skills: List[Skill] = Field(
        default_factory=list, description="Soft skills required for the position"
    )

    certifications: List[str] = Field(
        default_factory=list, description="Professional certifications required"
    )

    programming_languages: List[str] = Field(
        default_factory=list, description="Programming languages required"
    )

    frameworks_tools: List[str] = Field(
        default_factory=list, description="Frameworks and tools experience required"
    )
...
class EducationRequirement(BaseModel):
    ...
    level: EducationLevel = Field(
        EducationLevel.BACHELOR, description="Required education level"
    )

    field_of_study: Optional[str] = Field(None, description="Preferred field of study")

    is_required: bool = Field(
        True, description="Whether education requirement is mandatory"
    )
...
class SalaryRange(BaseModel):
    ...
    min_salary: Optional[Decimal] = Field(
        None, ge=0, description="Minimum salary amount"
    )

    max_salary: Optional[Decimal] = Field(
        None, ge=0, description="Maximum salary amount"
    )

    currency: str = Field("USD", description="Currency code (ISO 4217)")

    frequency: str = Field(
        "annual", description="Salary frequency (annual, monthly, hourly)"
    )

    is_negotiable: bool = Field(True, description="Whether salary is negotiable")

    equity_offered: bool = Field(
        False, description="Whether equity/stock options are offered"
    )

    bonus_structure: Optional[str] = Field(
        None, description="Description of bonus or commission structure"
    )
...
class JobRequirements(BaseModel):
    ...
    job_title: str = Field(
        ..., min_length=3, max_length=100, description="Job position title"
    )

    department: Optional[str] = Field(
        None, max_length=100, description="Department or team name"
    )
    ...
    salary: Optional[SalaryRange] = Field(
        None, description="Salary range and compensation details"
    )

    benefits: List[str] = Field(
        default_factory=list, description="Benefits and perks offered"
    )

    additional_requirements: Optional[str] = Field(
        None, max_length=1000, description="Additional requirements or notes"
    )
    ...
    def validate_job_consistency(self) -> "JobRequirements":
        ...
        if self.employment_type == EmploymentType.INTERNSHIP:
            if self.experience.years_min > 1 or self.experience.level not in [
                ExperienceLevel.ENTRY,
                ExperienceLevel.JUNIOR,
            ]:
                raise ValueError(
                    "Internships should not require significant experience"
                )

        # Validate senior roles have appropriate requirements (only if years_min > 0)
        if (
            self.experience.level
            in [ExperienceLevel.SENIOR, ExperienceLevel.LEAD, ExperienceLevel.PRINCIPAL]
            and self.experience.years_min > 0
            and self.experience.years_min < 3
        ):
            raise ValueError(
                f"{self.experience.level.value} roles typically require 3+ years experience"
            )
...
def validate_job_requirements(
    data: dict,
) -> tuple[Optional[JobRequirements], List[str]]:
