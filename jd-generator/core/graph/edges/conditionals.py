...
def should_generate_jd(state: GraphState) -> str:
    ...
    user_done_indicators = [
        "that's all", "i'm done", "finished", "complete", 
        "generate", "create jd", "no more"
    ]
    
    for msg in last_messages:
        if (msg.get("role") == "user" and 
            any(indicator in msg.get("content", "").lower() for indicator in user_done_indicators)):
            return "generate"
...
def route_to_next_question(state: GraphState) -> str:
    ...
    if (state.get("validation_errors") and 
...
def needs_clarification(state: GraphState) -> bool:
    ...
    ambiguous_responses = [
        "yes", "no", "maybe", "idk", "i don't know", 
        "not sure", "depends", "?"
    ]
    
    return (
        len(last_response.strip()) < 3 or
        last_response.lower().strip() in ambiguous_responses
    )
