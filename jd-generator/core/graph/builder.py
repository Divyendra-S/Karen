...
def create_jd_graph() -> StateGraph:
    ...
    from .nodes.evaluator import response_evaluator_node
    ...
    graph.add_edge("user_input_collector", "response_evaluator")
    graph.add_edge("response_evaluator", "state_updater")

    # Conditional edge from state_updater
    graph.add_conditional_edges(
        "state_updater",
        should_continue_collecting,
        {"continue": "question_router", "complete": "completeness_checker"},
    )
    ...
    graph.add_conditional_edges(
        ...
        should_generate_jd,
        {"generate": "jd_generator", "continue": "question_router"},
    )
...
def create_graph_config(
    max_retries: int = 3, timeout: int = 300, enable_checkpointing: bool = True
) -> Dict[str, Any]:
