...
def add_assistant_message(
    state: GraphState, content: str, message_type: str = "response"
) -> GraphState:
...
def get_last_user_message(state: GraphState) -> Optional[str]:
    ...
    """
    user_messages = [
        msg["content"] for msg in state["messages"] if msg.get("role") == "user"
    ]
...
def get_collected_fields(state: GraphState) -> List[str]:
    ...
    job_data_fields = [
        field
        for field, value in state["job_data"].items()
...
def validate_state_transition(
    current_state: GraphState, next_node: str
...
def add_evaluation_result(state: GraphState, evaluation_result: dict) -> GraphState:
    """Add evaluation result to the state.

    Args:
        state: Current graph state
        evaluation_result: LLM evaluation result
    ...
    return {
        **state,
        ...
        "extracted_fields": evaluation_result.get("extracted_fields", {}),
        "corrected_input": evaluation_result.get("corrected_input"),
        "needs_clarification": evaluation_result.get("needs_clarification", False),
    }


def clear_evaluation_data(state: GraphState) -> GraphState:
    """Clear evaluation data from state after processing.

    Args:
        state: Current graph state

    Returns:
        Updated state with cleared evaluation data
    """
    return {
        **state,
...
def mark_evaluation_failed(state: GraphState, error: str) -> GraphState:
    """Mark evaluation as failed and add error.
    ...
    Returns:
        Updated state with evaluation failure marked
    """
    return {
        **state,
...
def get_evaluation_summary(state: GraphState) -> Dict[str, Any]:
...
def safe_state_update(
    state: GraphState, update_function: Callable[[GraphState], GraphState]
) -> GraphState:
