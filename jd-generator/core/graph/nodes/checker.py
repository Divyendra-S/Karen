...
def completeness_checker_node(state: GraphState) -> Dict[str, Any]:
    ...
    if validation_errors:
        ...
        return {**state, "messages": new_messages, "validation_errors": validation_errors}
...
def create_completion_summary(
    ...
    summary_parts.append(f"**Employment Type**: {job_requirements.employment_type.value.replace('_', ' ').title()}")
    ...
    if job_requirements.benefits:
        summary_parts.append(f"**Benefits**: {len(job_requirements.benefits)} benefits listed")
...
def assess_data_quality(job_data: Dict[str, Any]) -> tuple[float, List[str]]:
    ...
    if not job_data.get("salary"):
        suggestions.append("Consider adding salary range to attract more qualified candidates")
    
    if not job_data.get("benefits"):
        suggestions.append("Adding benefits information can make the position more appealing")
...
def should_request_optional_fields(
    ...
    return len(prioritized_missing) > 0, prioritized_missing[:2]  # Max 2 optional fields