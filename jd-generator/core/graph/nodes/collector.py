...
def user_input_collector_node(state: GraphState) -> Dict[str, Any]:
    """Collect and process user input with preliminary validation.

    Pure function that collects user input and prepares it for evaluation.
    Now focuses on basic collection and flags for comprehensive evaluation.

    Args:
        state: Current graph state

    Returns:
        State updates with collected user input ready for evaluation
    """
    ...
    if not user_input or not current_field:
        return add_validation_error_to_state(
            state, "No user input or current field to process"
        )

    # Check if this appears to be a multi-field response
    is_multi_field = _detect_multi_field_response(user_input)

    # Check if response is too vague and needs immediate clarification
    if _is_response_too_vague(user_input):
        return _handle_vague_response(state, user_input)

    # Store user input for evaluation with metadata
    return {
        **state,
        "raw_user_input": user_input,
        "is_multi_field_response": is_multi_field,
...
def process_user_input(
    user_input: str, field_name: str, state: GraphState
) -> tuple[Any, List[str]]:
...
def process_background_intro(input_text: str) -> tuple[str, List[str]]:
    ...
    if len(input_text) < 10:
        errors.append(
            "Please tell me a bit more about yourself and your career interests."
        )
...
def process_location(
    input_text: str, state: GraphState
    ...
    if any(word in input_lower for word in ["remote", "work from home", "wfh"]):
        ...
        if not city:
            errors.append(
                "On-site positions need a city location. Please specify the city."
            )
...
def process_experience(input_text: str) -> tuple[Dict[str, Any], List[str]]:
    ...
    if any(word in input_lower for word in ["entry", "junior", "new grad", "0-2"]):
        ...
        word in input_lower for word in ["principal", "staff", "architect", "10+"]
    ):
        level = "principal"
    else:
        level = "mid"
...
def process_responsibilities(input_text: str) -> tuple[List[str], List[str]]:
    ...
    if len(input_text) < 20:
        errors.append(
            "Please provide more detailed responsibilities (at least 20 characters)."
        )
...
def process_education(input_text: str) -> tuple[Dict[str, Any], List[str]]:
    ...
    if any(word in input_lower for word in ["phd", "doctorate", "doctoral"]):
        ...
        word in input_lower for word in ["none", "not required", "no requirement"]
    ):
        level = "none_required"
    else:
...
def process_salary(input_text: str) -> tuple[Optional[Dict[str, Any]], List[str]]:
    ...
    if any(
        ...
        for word in ["skip", "not specify", "competitive", "negotiable"]
    ):
        return None, []
...
def _detect_multi_field_response(user_input: str) -> bool:
    """Detect if user response likely contains multiple field information.

    Args:
        user_input: User's response text

    Returns:
        True if response appears to contain multiple fields
    """
    # Check for multiple pieces of information
    multi_field_indicators = [
        # Job title and experience together
        ("years", "engineer"),
        ("years", "manager"),
        ("years", "developer"),
        ...
        ("remote", "experience"),
        ("city", "years"),
        # Multiple attributes in one response
        ("and", "also"),
        ("with", "plus"),
    ...
    indicator_count = sum(
        1
        for indicator1, indicator2 in multi_field_indicators
        if indicator1 in text_lower and indicator2 in text_lower
    )

    # Also check length - longer responses often contain multiple fields
    word_count = len(user_input.split())

    return indicator_count > 0 or word_count > 15


def _is_response_too_vague(user_input: str) -> bool:
    """Check if response is too vague to process effectively.

    Args:
        user_input: User's response text

    Returns:
        True if response is too vague
    """
    vague_responses = [
        "yes",
        "no",
    ...
    return (
        len(cleaned_input) < 3
        or cleaned_input in vague_responses
        or len(cleaned_input.split()) == 1
        and cleaned_input in ["yes", "no", "ok"]
    )


def _handle_vague_response(state: GraphState, user_input: str) -> Dict[str, Any]:
    """Handle vague responses that need immediate clarification.

    Args:
        state: Current graph state
        user_input: Vague user input

    Returns:
        State update with clarification request
    """
    current_field = state.get("current_field")

    clarification_message = (
        f"I'd like to understand your response better. "
        f"Could you provide more specific information about {current_field or 'your preferences'}? "
        "For example, share details, examples, or specific requirements you have in mind."
    )
    ...
    return {
        **state,
        ...
        "validation_errors": ["Response too vague, needs clarification"],
...
def add_validation_error_to_state(state: GraphState, error: str) -> GraphState:
    ...
    return {**state, "validation_errors": new_errors, "retry_count": new_retry_count}
