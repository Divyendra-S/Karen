...
def create_jd_header(job_requirements: JobRequirements) -> str:
    ...
    header_parts.append(f"**Employment Type:** {job_requirements.employment_type.value.replace('_', ' ').title()}")
...
def create_requirements_section(job_requirements: JobRequirements) -> str:
    ...
    if job_requirements.skills.technical_skills:
        tech_skills = [skill.name for skill in job_requirements.skills.technical_skills if skill.is_required]
        if tech_skills:
    ...
    if job_requirements.skills.soft_skills:
        soft_skills = [skill.name for skill in job_requirements.skills.soft_skills if skill.is_required]
        if soft_skills:
...
def create_qualifications_section(job_requirements: JobRequirements) -> str:
    ...
    preferred_technical = [
        skill.name for skill in job_requirements.skills.technical_skills 
    ...
    if job_requirements.skills.certifications:
        section += f"• Certifications: {', '.join(job_requirements.skills.certifications)}\n"
...
def categorize_job_role(job_title: str) -> str:
    ...
    if any(word in title_lower for word in ["engineer", "developer", "programmer", "architect"]):
        return "technical"
...
def create_jd_summary_stats(job_requirements: JobRequirements) -> Dict[str, Any]:
    ...
    return {
        ...
        "is_remote_friendly": job_requirements.location.location_type.value in ["remote", "hybrid"]
    }
