    ...
    def test_completion_percentage(self):
        ...
        job_req = JobRequirements(
            ...
            skills=SkillsRequirement(
                technical_skills=[Skill(name="Python", skill_type=SkillType.TECHNICAL)]
            ),
        )
...
class TestLocationRequirement:
    ...
    def test_travel_percentage_validation(self):
        ...
        with pytest.raises(
            ValueError, match="Travel percentage specified but travel not required"
        ):
            LocationRequirement(
...
class TestExperienceRequirement:
    ...
    def test_years_validation(self):
        ...
        with pytest.raises(
            ValueError, match="Maximum years cannot be less than minimum"
        ):
            ExperienceRequirement(level=ExperienceLevel.MID, years_min=5, years_max=3)
    ...
    def test_level_years_consistency(self):
        ...
        with pytest.raises(ValueError, match="entry level typically requires"):
            ExperienceRequirement(
                level=ExperienceLevel.ENTRY, years_min=5  # Entry level max is 2 years
            )
...
class TestSalaryRange:
    ...
    def test_salary_validation(self):
        ...
        with pytest.raises(ValueError, match="Minimum salary cannot exceed maximum"):
            SalaryRange(min_salary=Decimal("100000"), max_salary=Decimal("80000"))

    def test_computed_properties(self):
        """Test computed salary properties."""
        salary = SalaryRange(min_salary=Decimal("80000"), max_salary=Decimal("120000"))
...
class TestConversationModels:
    ...
    def test_message_creation(self):
        ...
        message = Message(role=MessageRole.USER, content="Software Engineer")
...
class TestFunctionalValidation:
    ...
    def test_validate_job_requirements_success(self):
        ...
        data = {
            ...
            "responsibilities": ["Develop software"],
            "skills": {
                "technical_skills": [{"name": "Python", "skill_type": "technical"}]
            },
        }
...
@pytest.mark.parametrize(
    "location_type", [LocationType.ON_SITE, LocationType.REMOTE, LocationType.HYBRID]
)
